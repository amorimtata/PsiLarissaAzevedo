...
export const AboutCarousel = () => {
  // --- ⚙️ MANTIDO PARA FUTURO USO (CASO VOLTE O CARROSSEL) ---
  /*
  const [emblaRef, emblaApi] = useEmblaCarousel({ loop: true });
  const [selectedIndex, setSelectedIndex] = useState(0);
  ...
  // Autoplay (comentado para deixar só a primeira imagem fixa)
  useEffect(() => {
    if (!emblaApi) return;
    const interval = setInterval(() => {
      emblaApi.scrollNext();
  ...
  */
  ...
  // --- 🌿 NOVA VERSÃO: apenas a primeira imagem (pho5) aparece ---
  return (
    <div className="relative">
      <div className="absolute inset-0 bg-primary/10 rounded-3xl transform rotate-3"></div>
